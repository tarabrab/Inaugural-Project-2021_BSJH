...
def solve_func(m,  par_dict):
    ...
    Returns:
       Consumption and housing that solves the maximization problem.
    ...
    #Defining bounds - maximum are maximal consumption levels - upper bound for housing is set 
    #in the case where the price is as low as possible thus when p_tilde is smaller than pbar. 
...
def calc_rev(mean,var, N, par_dict):
    """ This function calculates average tax revenue for a given distribution - log-normal - as a function of,
...
def calc_rev_5(tg_set, par_dict, draw, N):
    """ This function calculates average tax revenue for a given distribution - log-normal - as a function of
        distribution mean, distribution variance, number of draws and given parameters. tg is set outside the dictionary
        in order to be able to use it as a minimization argument. 
    Args:
         mean: Distribution mean
         par_dict: Dictionary with parameter values
         var: Distribution variance
